<script>
  ...
  import MonitorCharts from "./MonitorCharts.svelte";
  ...
  async function refreshCurveStatus() {
    try {
      const raw = await invokeTauri("send_command", {
  ...
  async function copyJson() {
    ...
    try {
      ...
      setTimeout(() => {
        statusMessage = agentStatus.value === "running" ? "Running" : "Stopped";
      }, 1500);
    } catch (_) {
...
<div class="min-h-screen bg-[#0d1117] flex flex-col">
  <!-- Header -->
  <header
    class="border-b border-[#30363d] px-6 py-3 flex items-center gap-4 shrink-0"
  >
    <h1 class="font-mono text-xl font-bold text-[#58a6ff] tracking-tight">
      omenctl
    </h1>
    ...
    <span class="text-[#8b949e] text-xs ml-4"
      >Device: {deviceName} | Profile: {profileId}</span
    >
    ...
    {#if snapshot.data}
      ...
      <section class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
        <SensorCard
          ...
          value={snapshot.data.temps?.cpu?.value}
          unit={snapshot.data.temps?.cpu?.unit}
          source={snapshot.data.temps?.cpu?.source}
          trusted={snapshot.data.temps?.cpu?.trusted}
          suspect={snapshot.data.temps?.cpu?.suspect}
          kind="temperature"
        />
        <SensorCard
          label="GPU Temperature"
          value={snapshot.data.temps?.gpu?.value}
          unit={snapshot.data.temps?.gpu?.unit}
          source={snapshot.data.temps?.gpu?.source}
          trusted={snapshot.data.temps?.gpu?.trusted}
          suspect={snapshot.data.temps?.gpu?.suspect}
          kind="temperature"
        />
        <SensorCard
          label="CPU Load"
          value={snapshot.data.loads?.cpu?.value}
          unit={snapshot.data.loads?.cpu?.unit}
          source={snapshot.data.loads?.cpu?.source}
          trusted={snapshot.data.loads?.cpu?.trusted}
          suspect={snapshot.data.loads?.cpu?.suspect}
        />
        <SensorCard
          label="GPU Load"
          value={snapshot.data.loads?.gpu?.value}
          unit={snapshot.data.loads?.gpu?.unit}
          source={snapshot.data.loads?.gpu?.source}
          trusted={snapshot.data.loads?.gpu?.trusted}
          suspect={snapshot.data.loads?.gpu?.suspect}
        />
      </section>
      ...
      <section class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
        {#if snapshot.data.loads?.memory}
          <SensorCard
            label="System Memory"
            value={snapshot.data.loads.memory.value}
            unit={snapshot.data.loads.memory.unit}
            source={snapshot.data.loads.memory.source}
            trusted={snapshot.data.loads.memory.trusted}
            suspect={snapshot.data.loads.memory.suspect}
          />
        {/if}
        {#if snapshot.data.raw?.sensors?.gpuPowerW}
          <SensorCard
            label="GPU Power"
            value={snapshot.data.raw.sensors.gpuPowerW.value}
        ...
        {#if snapshot.data.raw?.sensors?.gpuCoreClockMHz}
          <SensorCard
            ...
            value={snapshot.data.raw.sensors.gpuCoreClockMHz.value}
