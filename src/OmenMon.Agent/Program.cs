...
async Task<AgentResponse> DispatchAsync(AgentCommand command, CancellationToken cancellationToken)
{
    ...
    try
    {
        return command.Cmd switch
        {
            ...
            "setAuto" => AgentResponse.Success(await controller.SetAutoAsync(command.BiosMode, cancellationToken).ConfigureAwait(false)),
            "setMax" => AgentResponse.Success(await controller.SetMaxAsync(cancellationToken).ConfigureAwait(false)),
            "setManual" => AgentResponse.Success(await SetManualAsync(command, cancellationToken).ConfigureAwait(false)),
            "setProgram" => AgentResponse.Success(await SetProgramAsync(command, cancellationToken).ConfigureAwait(false)),
