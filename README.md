...
项目从开源 [OmenMon](https://omenmon.github.io/) 代码演化而来，但目标已经不再是维护原版 WinForms GUI，而是一套更清晰的现代本地 Agent：
...
OmenMon.Modern.sln
src/
  OmenMon.Core/      核心协议、设备画像、快照模型、控制接口
  OmenMon.Sensors/   传感器 provider 层，后续接 LHM / NVML
  OmenMon.Agent/     JSON over stdio 本地 Agent
docs/
  TODO.md            当前开发计划
```
...
- `OmenMon.Core` 已接入最小 BIOS / EC / fan control 路径。
- Agent 已支持实验性风扇曲线后台循环：读取可信温度，按曲线调用 `setManual`。
...
1. 接入 LibreHardwareMonitor。
2. 接入 NVML。
3. 输出可信的统一 `snapshot`。
4. 用 LHM/NVML 温度驱动风扇曲线。
5. 再决定是否用 Tauri/Web 重写 UI。
